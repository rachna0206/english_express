    ...
    try
    {
      for($i=1;$i<$total;$i++){
        ...
        if($type=='current'){
          ...
          $stmt = $obj->con1->prepare("update attendance set remark=? where aid=?");
          $stmt->bind_param("si", $remark,$aid);
          $Resp=$stmt->execute();
        }
          ...
          ?>
                    ...
                    <div class="mb-3">
                          ...
                          <div class="form-check form-check-inline mt-3">
                            <input class="form-check-input" type="radio" name="stu_type" id="stu_type_all" value="all" onchange="showGrid(frm1)"  <?php echo ($_REQUEST['stu_type']!="" && $_REQUEST['stu_type']=="all")?"checked":""?> >
                            <label class="form-check-label" for="inlineRadio1">All</label>
                          </div>
                          <div class="form-check form-check-inline mt-3">
                            <input class="form-check-input" type="radio" name="stu_type" id="stu_type_current" value="current" onchange="showGrid(frm1)"  <?php echo ($_REQUEST['stu_type']!="" && $_REQUEST['stu_type']=="current")?"checked":""?> >
            ...
            <!-- Basic Bootstrap Table -->
              <div class="card">
                ...
                if(isset($_REQUEST['stu_type']))
                {
                  ...
                  $stu_str=($stu_type!="")?" and b1.student_status not in ('on_hold','dismiss','course_completed_with_exam','course_completed_without_exam','course_completed','batch_change')":"";
                  ?>
                <div class="table-responsive text-nowrap">
                  <table class="table">
                    ...
                    //select a.*,s.name from attendance a, student s,batch_assign b1 where a.student_id=s.sid and b1.student_id=a.student_id and a.batch_id=b1.batch_id and a.batch_id =13 and a.dt ='2023-01-10' and b1.student_status not in ('on_hold','dismiss','course_completed_with_exam','course_completed_without_exam','course_completed','batch_change') group by a.student_id
                      ...
                      $stmt_list = $obj->con1->prepare("select a.*,s.name from attendance a, student s,batch_assign b1 where a.student_id=s.sid and b1.student_id=a.student_id and a.batch_id=b1.batch_id and a.batch_id = '".$batch_id."' and a.dt ='".$dt."'".$stu_str." group by a.student_id");
                      ...
                      $i=1;
                        ...
                        <?php
                                ...
                                <tr>
                                  ...
                                  <?php
                                    if($_REQUEST["stu_type"]==""){
                                      $stype = "all";
                                    } else{
                                      $stype = $_REQUEST["stu_type"];
                                    }
                                    if($stype=='current'){
                                  ?>
                                  <td>
                                  ...
                                  ?>
                                  <td style="color:<?php echo ($a["faculty_attendance"]=="p")?'green':'red' ?>"><?php echo ucfirst($a["faculty_attendance"])?></td>
                                  <input type="hidden" name="atten_id<?php echo $i ?>" value="<?php echo $a["aid"] ?>">
                                  <input type="hidden" name="type" value="<?php echo $stype ?>">
                                  <?php
                                    }
                                  ...
                                  <td><select class="form-control" name="status<?php echo $i?>">
                                    ...
                                    <option value="ongoing" <?php echo ($res_batch_assign["student_status"]=="ongoing")?"selected":""?>>Regular</option>
                                  </select></td>
